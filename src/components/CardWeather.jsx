  ...
  useEffect(() => {
    if (tempUnit === 'f') {
      setTemperature(
        `${
          Math.round(
            (((parseFloat(weather?.main.temp) - 273.15) * 9) / 5 + 32) * 100
          ) / 100
        }  °F`
      );
    } else {
      setTemperature(
        `${
          Math.round((parseFloat(weather?.main.temp) - 273.15) * 100) / 100
        }  °C`
      );
    }
  }, [tempUnit]);
        ...
        <div className="card">
          ...
          <p className="temperature">
            {tempUnit == 'c'
              ? `${
                  Math.round(
                    (((parseFloat(weather?.main.temp) - 273.15) * 9) / 5 + 32) *
                      100
                  ) / 100
                } °F`
              : `${
                  Math.round((parseFloat(weather?.main.temp) - 273.15) * 100) /
                  100
                } °C`}
          </p>
